PlayerJob, onDuty, Peds, Targets, searchProps, Props = {}, false, {}, {}, {}, {}
				...
				for i = 1, #jobLoc.Trade do
					...
					Targets[nameTrade] =
						exports['qb-target']:AddBoxZone(nameTrade, vec3(jobLoc.Trade[i].coords.x, jobLoc.Trade[i].coords.y, jobLoc.Trade[i].coords.z-1), jobLoc.Trade[i].w, jobLoc.Trade[i].d, { name=nameTrade, heading = jobLoc.Trade[i].coords.w, debugPoly=Config.Debug, minZ= jobLoc.Trade[i].coords.z-1, maxZ=jobLoc.Trade[i].coords.z+1 },
							{ options = { { event = "jim-recycle:Trade:Menu", icon = "fas fa-box", label = Loc[Config.Lan].target["trade"], job = loc.Job }, },
							distance = 1.5 })
				end
			end
		end
...
RegisterNetEvent("jim-recycle:PickupPackage:Start", function(data) local Ped = PlayerPedId()
	...
	if progressBar({label = Loc[Config.Lan].progressbar["search"], time = Config.Debug and 1000 or 5000, cancel = true, icon = "fas fa-magnifying-glass"}) then
...
RegisterNetEvent("jim-recycle:PickupPackage:Hold", function(data) local Ped = PlayerPedId()
    ...
    TaskPlayAnim(Ped, "anim@heists@box_carry@", "idle", 5.0, -1, -1, 50, 0, 0, 0, 0)
	v = Config.scrapPool[math.random(1, #Config.scrapPool)]
    scrapProp = makeProp({prop = v.model, coords = vec4(0,0,0,0)}, 1, 1)
...
RegisterNetEvent('jim-recycle:dutytoggle', function(data)
	if Config.JobRole then
		...
		onDuty = not onDuty
		if onDuty then TriggerEvent('QBCore:Notify', Loc[Config.Lan].success["on_duty"], 'success') PickRandomPackage(data.Trolly)
...
RegisterNetEvent('jim-recycle:SellAnim', function(data) local Ped = PlayerPedId()
	...
	for k in pairs(Config.Prices) do
		if k == data.item then TriggerServerEvent('jim-recycle:Selling:Mat', data.item) Selling = false return end
	end
	TriggerServerEvent("jim-recycle:TradeItems", data.item)
	Selling = false
end)
...
RegisterNetEvent('jim-recycle:Selling:Menu', function(data)
	...
	for item, price in pairsByKeys(Config.Prices) do
		sellMenu[#sellMenu+1] = {
			...
			header = Core.Shared.Items[item].label,
			txt = Loc[Config.Lan].menu["sell_all"]..price..Loc[Config.Lan].menu["each"],
...
RegisterNetEvent('jim-recycle:Bottle:Menu', function()
	...
	tradeMenu[#tradeMenu+1] = { disabled = not HasItem("can", 1), icon = "can", header = "<img src=nui://"..Config.img..Core.Shared.Items["can"].image.." width=30px onerror='this.onerror=null; this.remove();'> "..Core.Shared.Items["can"].label, params = { event = "jim-recycle:SellAnim", args = 'can' } }
	tradeMenu[#tradeMenu+1] = { disabled = not HasItem("bottle", 1), icon = "bottle", header = "<img src=nui://"..Config.img..Core.Shared.Items["bottle"].image.." width=30px onerror='this.onerror=null; this.remove();'> "..Core.Shared.Items["bottle"].label, params = { event = "jim-recycle:SellAnim", args = 'bottle' } }
